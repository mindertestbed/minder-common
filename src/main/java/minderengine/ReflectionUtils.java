...
public class ReflectionUtils {
  ...
  /**
   * A validator that assumes that an array identifier would be like xyz.Abc[ ] or int[]
   * <br>
  ...
  public static synchronized boolean isValidJavaType(String str) {
    ...
    if (str.matches(JAVA_TYPE_EXP)
        && !str.matches(STARTS_WITH_KEYWORD) && !str.matches(ENDS_WITH_KEYWORD) && !str.matches(INCLUDES_KEYWORD)) {
      return true;
    }
  ...
  /**
   * converts the cannonical name to a Java class
   *
   * @param cannonical The class descriptor (might be a primitive, an array and a class)
   * @return the class that is resolved
   */
  public static Class<?> cannonical2Class(String cannonical) {
    ...
    if (cannonical.contains("[")) {
      //get the part before [
      String firstPart = cannonical.substring(0, cannonical.indexOf('[')).trim();
      ...
      if (primitiveArrayMap.containsKey(firstPart)) {
        try {
          return Class.forName(outer.append(primitiveArrayMap.get(firstPart)).toString());
        } catch (ClassNotFoundException e) {
          throw new IllegalArgumentException(cannonical + " is not valid in this context", e);
        }
      } else {
        //not primitive
        try {
          String className = outer.append('L').append(new String(canArray, 0, firstBracketIndex)).append(';').toString();
